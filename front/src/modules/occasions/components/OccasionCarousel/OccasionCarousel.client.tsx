  ...
  return (
    <div className="py-4">
      <Swiper
        dir="rtl"
        spaceBetween={12}
        className="[&_.swiper-wrapper]:flex"
        breakpoints={{
          0: {
            slidesPerView: 1.4,
      ...
      >
        {loading
          ? Array.from({ length: SKELETON_COUNT }).map((_, i) => (
              ...
              >
                <div
                  className="flex flex-col items-center text-center p-4 pb-3 min-h-[176px] rounded-xl lg:min-h-[200px] bg-[var(--surface-surface-soft)] relative overflow-hidden h-[212px] animate-pulse"
                  aria-busy="true"
                  aria-label="در حال بارگذاری"
                >
                  <div className="w-12 h-6 rounded-lg bg-gray-200" />
                  <div className="w-16 h-3.5 mt-1.5 rounded-lg bg-gray-200" />
                  <div className="w-[70%] h-4 mt-3 rounded-lg bg-gray-200" />
                  <div className="w-1/2 h-3 mt-2 rounded-lg bg-gray-200" />
                  <div className="w-24 h-7 mt-3 rounded-full bg-gray-200" />
                </div>
              </SwiperSlide>
            ))
          : occasions.map((o, idx) => (
