...
import type { CallOptions } from "./payment.types"

const DEFAULT_TIMEOUT_MS = 8_000
const DEFAULT_RETRIES = 3

export function resolveSandbox(): boolean {
...
export function getZarinpalClient(): ZarinPal {
...
export function ensureMerchantId(): string {
...
export function mapSdkError(err: unknown, operation: string): never {
...
export async function withZarinpalRetry<T>(
  operation: string,
...
export function toSdkMobile(mobile?: string | null): string | undefined {
...
export function toSdkEmail(email?: string | null): string | undefined {
  const trimmed = (email || "").trim()
  if (!trimmed) return undefined
  return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(trimmed) ? trimmed : undefined
}

export function assertAbsoluteCallbackUrl(callbackUrl: string): void {
  try {
...
export function resolveZarinpalCurrency(currency?: "IRT" | "IRR"): "IRT" | "IRR" {
