...
type OccasionsAction = { method: "GET" | "POST" | "PATCH" | "DELETE"; path: string; body?: unknown; response: "list" | "item" };
export async function executeOccasions(action: OccasionsAction, requestId: string) {
  const response = await wordpressFetch(action.path, { method: action.method, body: action.body ? JSON.stringify(action.body) : undefined, headers: { ...await wordpressBearerHeaders(), ...(action.body ? { "Content-Type": "application/json" } : {}) }, cache: "no-store", requestId });
