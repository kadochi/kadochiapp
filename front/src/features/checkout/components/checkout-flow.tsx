...
function deliveryDate(date: string, isToday = false) {
  const value = new Date(`${date}T12:00:00Z`);
  return { weekday: isToday ? "امروز" : faWeekday.format(value), date: faDate.format(value) };
}
    ...
    <section className="px-16 pb-16">
      ...
      <div className="mt-8 grid grid-cols-4 gap-8">
        {deliveryDays.map((day, index) => {
          ...
          const date = deliveryDate(day.date, index === 0);
          return <button
            key={day.date}
