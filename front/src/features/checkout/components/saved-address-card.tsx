...
function AddressDetails({ address }: { address: SavedAddress }) {
  return <span className="grid gap-4"><span className="text-title-14 font-bold">{address.title}</span><span className="text-label-12 leading-20 text-surface-neutral-mid-emphasis">{["تهران", address.address1, address.address2].filter(Boolean).join("، ")}</span></span>;
}
