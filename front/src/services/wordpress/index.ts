...
function parseWithSchema<T>(raw: unknown, options: WordPressJsonOptions<T>): T {
  if (options.schema) {
...
export async function wordpressFetch(
  ...
  const key =
    options.dedupeKey ||
    (idempotent ? `${method}:${url.toString()}` : undefined);
...
export async function wordpressJson<T>(
  ...
  const key =
    options.dedupeKey ||
    (idempotent
      ? `${method}:${buildWordPressURL(input).toString()}`
      : undefined);
  ...
  const runner = (async () => {
    ...
    if (response.status === 304) {
      return {
        ...
        etag,
        notModified: true,
      } satisfies WordPressJsonResult<T>;
    }
  ...
  if (key)
    inflightJson.set(key, runner as Promise<WordPressJsonResult<unknown>>);
